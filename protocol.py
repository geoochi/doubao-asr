...
def setup_logging(log_dir: str = 'log') -> None:
    """配置日志：按启动时间命名文件，存放在 log 目录下。

    仅在作为脚本运行时调用，避免 import 时产生副作用（写文件、改全局 logger 配置）。
    """
    import os
    ...
    logger.remove()
    logger.add(
        f'{log_dir}/asr_{{time:YYYY-MM-DD_HH-mm-ss}}.log', level='INFO', encoding='utf-8'
    )
...
class RequestBuilder:
    @staticmethod
    def new_auth_headers(config) -> dict[str, str]:
    ...
    @staticmethod
    def new_full_client_request(seq: int, payload) -> bytes:  # 添加seq参数
...
class AsrWsClient:
    ...
    async def create_connection(self, config) -> None:
        headers = RequestBuilder.new_auth_headers(config)
        try:
    ...
    async def send_full_client_request(self, payload) -> None:
    ...
    async def execute(self, file_path: str, config, payload) -> AsyncGenerator[AsrResponse, None]:
        if not file_path:
        ...
        try:
            ...
            # 4. 发送完整客户端请求
            await self.send_full_client_request(payload)
